...
function AProfile(){
    ...
    async  function stop(event){
      ...
      useEffect(() => {
        const fetchData = async () => {
            const response = await fetch("http://localhost:3001/api/alltotal");
            const data = await response.json();
            setAll(data);
        }
        fetchData();
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await fetch("http://localhost:3001/api/student1");
            const data = await response.json();
            if(data){
            setData(data);
   ...
   return result;
      ...
      if(!removedep){
        if(window.confirm("Are you sure you want to remove this Department?")){
          ...
          setSelectedDept(selectDept.filter(item => item !== i));
           ...
           axios.post("http://localhost:3001/removeDept",{arr}).then((response) =>{
    ...
    const submitFile = async (event) => {
      ...
      await axios.post('http://localhost:3001/pdfnotify', formData, {
    ...
    function handleattsheet(event){
     ...
     if(jsonData){
      ...
      axios.post("http://localhost:3001/updateusers", {jsonData }).then((response) => {
        setLoad(true);
    ...
    function inputnotify(event){
      ...
      if(input){
        if(window.confirm("Are you sure, you want to send the notification?")){
          let date1 = new Date();
          let date2 = date1.toLocaleDateString() + " " + date1.toLocaleTimeString();
          date2 = new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString();
         setDate(date2);
          axios.post("http://localhost:3001/notify",{input,date}).then((response) =>{
