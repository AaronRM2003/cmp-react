    ...
    const fetchFile = async () => {
      ...
      for (let fil of data.assignment[sub]) {
        try {
          ...
          const response = await axios(`http://localhost:3001/getFile/${f}`, {
            method: 'GET',
            responseType: 'blob',
...
// The original filename is all parts except the last one
  ...
  const sortedDates = Object.keys(groupedByDate).sort();
    ...
    async function handleassig(event){
      ...
      if(assig && 0 < fromMonth && fromMonth <= 31 && 0 < fromDay && fromDay < 13 && 2020 < fromYear && fromYear < 2199){
        setLoad(true);
       ...
       cnt =data.assignment[sub].length;
        ...
        axios.post("http://localhost:3001/assig", { assig,date,date1,filenameWithSuffix,selectedValue }).then((response) => {
          setLoad(false);
        ...
        await axios.post('http://localhost:3001/upload', formData, { headers })
          .then((response) => {
            setLoad(false);
