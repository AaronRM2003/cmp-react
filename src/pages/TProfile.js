 ...
 function TProfile() {
    ...
    function submitpass(event){
        ...
        if(npass === cpass){
          if(window.confirm("Are you ready to Submit?")){
            setShow(true);
            axios.post("http://localhost:3001/changepass",{cpass}).then((response)=>{
  ...
  /*  useEffect(() => {
    ...
    You are not logged in.
        ...
        <h2 className={isDesktop ? "attcont3" : "attcont2"}>WELCOME  {data ? data.name : 'Loading...'}</h2>
        ...
        <div >
             <h2 className={isDesktop ? "attcont3" : "attcont2"}>Your Class:{data ? " "+data.class : 'Loading...'}</h2>       
             <h2 className={isDesktop ? "attcont3" : "attcont2"}>Subject:{data ? " "+data.subject : 'Loading...'}</h2>
        </div>
        <div  className={isDesktop ? "attcont3" : "attcont2"}>
